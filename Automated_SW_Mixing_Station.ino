...
int TankStatusRead(){
  ...
  ping = HCSR04Read(Sens1_trigPin, Sens1_echoPin);
  //Calculate the water level in the tank
  TankLevel = TankHeight - ping;
  ...
  if (ping > 2000)      // HC-SR04 is directly up against what it is measuring
    level = HighHigh;
  if (ping == 0)        // HC-SR04 has failed or is disconnected
    level = HighHigh;
